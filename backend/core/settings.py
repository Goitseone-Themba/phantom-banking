...
# Add debug toolbar only in development (but not during tests)
import sys
...
MIDDLEWARE = [
    ...
    'core.middleware.UUIDValidationErrorMiddleware',  # Handle UUID validation errors
...
# Add debug toolbar middleware only in development (but not during tests)
if DEBUG and 'test' not in sys.argv:
...
# Add throttling only when not running tests
import sys
...
REST_AUTH_SERIALIZERS = {
    ...
    'PASSWORD_RESET_SERIALIZER': 'phantom_apps.authentication.serializers.PasswordResetSerializer',
    'PASSWORD_RESET_CONFIRM_SERIALIZER': 'phantom_apps.authentication.serializers.PasswordResetConfirmSerializer',
    'PASSWORD_CHANGE_SERIALIZER': 'phantom_apps.authentication.serializers.PasswordChangeSerializer',
}

REST_AUTH_REGISTER_SERIALIZERS = {
    'REGISTER_SERIALIZER': 'phantom_apps.authentication.serializers.UserRegistrationSerializer',
}
...
# Override email settings if not in debug mode
if not DEBUG:
    EMAIL_HOST = env('EMAIL_HOST', default='')
    EMAIL_PORT = env('EMAIL_PORT', default=587)
    EMAIL_USE_TLS = env('EMAIL_USE_TLS', default=True)
