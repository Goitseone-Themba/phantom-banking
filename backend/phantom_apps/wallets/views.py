...
@extend_schema(
    tags=['Wallets'],
    summary='Wallets API information',
    description='''
    Root endpoint for Wallets API providing information about available endpoints.
    
    **Available Operations:**
    - Create customer wallets
    - Retrieve customer wallet details
    - List merchant wallets with pagination
    - Verify data integrity
    ''',
    responses={
        200: {
            'description': 'Wallets API information',
            'example': {
                'api': 'Phantom Banking Wallets API',
                'version': '1.0.0',
                'description': 'Digital wallet management for embedded banking',
                'endpoints': {
                    'create_customer_wallet': '/api/v1/wallets/merchants/me/customers/{customer_id}/wallet/',
                    'get_customer_wallet': '/api/v1/wallets/customers/{customer_id}/wallet/',
                    'list_merchant_wallets': '/api/v1/wallets/merchants/{merchant_id}/wallets/',
                    'verify_data_integrity': '/api/v1/wallets/merchants/{merchant_id}/integrity/'
                }
            }
        }
    }
...
def wallets_api_root(request):
    """
    Wallets API root endpoint with available operations information
    
    URL: GET /api/v1/wallets/
    """
    try:
        base_url = request.build_absolute_uri('/api/v1/wallets/')
        ...
        return Response({
            ...
            'endpoints': {
                'create_customer_wallet': {
                    'url': f"{base_url}merchants/me/customers/{{customer_id}}/wallet/",
                    'method': 'POST',
                    'description': 'Create or get phantom wallet for customer',
                    'authentication': 'JWT required'
                },
                'get_customer_wallet': {
                    'url': f"{base_url}customers/{{customer_id}}/wallet/",
                    'method': 'GET',
                    'description': 'Retrieve customer wallet details',
                    'authentication': 'JWT required'
                },
                'list_merchant_wallets': {
                    'url': f"{base_url}merchants/{{merchant_id}}/wallets/",
                    'method': 'GET',
                    'description': 'List all wallets for a merchant with pagination',
                    'authentication': 'JWT required'
                },
                'verify_data_integrity': {
                    'url': f"{base_url}merchants/{{merchant_id}}/integrity/",
                    'method': 'GET',
                    'description': 'Verify data integrity for merchant customers and wallets',
                    'authentication': 'JWT required'
                }
            },
            'features': [
        ...
        logger.error(f"Wallets API root view error: {e}")
        return Response({
            'error': 'Unable to generate wallets API information',
            'status': 'error'
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
