...
def process_qr_payment_merchant(request):
    ...
    try:
        ...
        logger.info(f"QR Payment successful - Merchant: {merchant.id}, Transaction: {transaction_obj.transaction_id}")
        ...
        return Response({
            'error': True,
            'error_code': PhantomBankingErrorCodes.INTERNAL_SERVER_ERROR,
...
def process_eft_payment_merchant(request):
    ...
    try:
        ...
        # Validate request data
        serializer = EFTPaymentSerializer(data=request.data)
        if not serializer.is_valid():
            return Response({
                'error': True,
                'error_code': PhantomBankingErrorCodes.INVALID_TRANSACTION_DATA,
        ...
        logger.info(f"EFT Payment successful - Merchant: {merchant.id}, Transaction: {transaction_obj.transaction_id}")
