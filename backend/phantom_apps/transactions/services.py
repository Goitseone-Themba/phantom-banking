...
class QRCodeService:
    ...
    def process_qr_payment(qr_code: QRCode, customer_phone: str, 
        ...
        try:
            ...
            if wallet_id:
                wallet = Wallet.objects.get(wallet_id=wallet_id, customer=customer)
            else:
    ...
    def process_webhook(external_reference: str, webhook_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                # Create a copy of response_data to avoid mutating original
                updated_response_data = dict(eft_payment.response_data or {})
                # Convert Decimal and datetime objects to strings for JSON serialization
                serializable_webhook_data = {}
                for key, value in webhook_data.items():
                    if isinstance(value, Decimal):
                        serializable_webhook_data[key] = str(value)
                    elif hasattr(value, 'isoformat'):  # datetime objects
                        serializable_webhook_data[key] = value.isoformat()
                    else:
                        serializable_webhook_data[key] = value
                updated_response_data.update(serializable_webhook_data)
                eft_payment.response_data = updated_response_data
