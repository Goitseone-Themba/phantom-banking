...
class InsufficientFundsError(PhantomBankingException):
    """Exception for insufficient funds in wallet"""
    def __init__(self, wallet_id=None, amount=None):
        if wallet_id and amount:
            message = f"Insufficient funds in wallet {wallet_id} for transaction of {amount}"
        else:
            message = "Insufficient funds in wallet for this transaction"
        super().__init__(
            message=message,
            error_code=PhantomBankingErrorCodes.INSUFFICIENT_FUNDS,
